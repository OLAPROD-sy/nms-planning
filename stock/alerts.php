...
<div class="alerts-container">
    ...
    <h1 style="margin-bottom: 20px; font-size: 1.5em;"><i class="bi bi-exclamation-triangle"></i> Tableau de Bord des Alertes</h1>
    </div>
    ...
    <form method="GET" style="display: flex; align-items: center; gap: 10px; width: 100%;">
        <label style="font-weight: bold; color: var(--dark);"><i class="bi bi-geo-alt"></i> Filtrer par site :</label>
        ...
        <?php if($filter_site): ?>
            <a href="alerts.php" style="color: var(--danger); text-decoration: none; font-size: 0.9em;"><i class="bi bi-x"></i> Réinitialiser</a>
        <?php endif; ?>
    </form>
</div>
    <div class="kpi-grid">
        ...
        <div class="kpi-card" style="border-left-color: var(--success);">
            <h3>État Global</h3>
            <div class="value" style="font-size: 1.2em;"><?= count($alertes) === 0 ? '<i class="bi bi-check-circle"></i> Sain' : '<i class="bi bi-exclamation-diamond"></i> Action Requise' ?></div>
        </div>
    </div>

    <div class="alert-section">
        <h2 style="color: var(--danger);"><i class="bi bi-exclamation-octagon"></i> Produits Critiques</h2>
        <?php if (empty($alertes)): ?>
            <p style="color: var(--success); margin-top:10px;"><i class="bi bi-check-circle"></i> Aucun produit sous le seuil d'alerte.</p>
        <?php else: ?>
                ...
                <table class="custom-table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span style="color:#7f8c8d; font-size: 0.85em;"><i class="bi bi-geo-alt"></i> <?= htmlspecialchars($sites_map[$a['id_site']] ?? 'Global') ?></span></td>
                            <td>
    ...
    <div class="alert-section">
        <h2 style="color: var(--warning);"><i class="bi bi-exclamation-circle"></i> Stocks Faibles (< 10)</h2>
        <div class="table-responsive">
            <table class="custom-table">
                <thead>
                    <tr>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><span style="color:#7f8c8d; font-size: 0.85em;"><i class="bi bi-geo-alt"></i> <?= htmlspecialchars($sites_map[$s['id_site']] ?? 'Global') ?></span></td>
                        <td style="font-weight:bold"><?= $s['quantite_actuelle'] ?></td>
                        <td>
                            <span class="badge-status <?= $is_critique ? 'badge-critique' : 'badge-faible' ?>">
                                <?= $is_critique ? '<i class="bi bi-exclamation-octagon"></i> Critique' : '<i class="bi bi-exclamation-circle"></i> Faible' ?>
                            </span>
                        </td>
                    </tr>
                    <?php endforeach; ?>
    ...
    <div style="margin-top: 10px;">
        <a class="btn-main" href="/stock/gest_stock.php" style="background: var(--success); text-decoration:none; display:inline-block; padding: 15px 25px; color:white; border-radius:8px; font-weight: bold;"><i class="bi bi-box-seam"></i> Gérer les stocks</a>
    </div>
</div>
